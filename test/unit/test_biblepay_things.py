...
def test_valid_biblepay_address():
    from biblepaylib import is_valid_biblepay_address

    main = valid_biblepay_address()
    test = valid_biblepay_address('testnet')

    assert is_valid_biblepay_address(main) is True
    assert is_valid_biblepay_address(main, 'mainnet') is True
    assert is_valid_biblepay_address(main, 'testnet') is False

    assert is_valid_biblepay_address(test) is False
    assert is_valid_biblepay_address(test, 'mainnet') is False
    assert is_valid_biblepay_address(test, 'testnet') is True


def test_invalid_biblepay_address():
    from biblepaylib import is_valid_biblepay_address

    main = invalid_biblepay_address()
    test = invalid_biblepay_address('testnet')

    assert is_valid_biblepay_address(main) is False
    assert is_valid_biblepay_address(main, 'mainnet') is False
    assert is_valid_biblepay_address(main, 'testnet') is False

    assert is_valid_biblepay_address(test) is False
    assert is_valid_biblepay_address(test, 'mainnet') is False
    assert is_valid_biblepay_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from biblepaylib import elect_mn
...
def test_parse_masternode_status_vin():
    from biblepaylib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % biblepaylib.hashit(sb_data_hex)
