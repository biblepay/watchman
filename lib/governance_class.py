...
class GovernanceClass(object):
    ...
    def vote(self, biblepayd, signal, outcome):
        return self.go.vote(biblepayd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, biblepayd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(biblepayd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(biblepayd, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to biblepayd "gobject list" output
    ...
    def biblepayd_serialise(self):
        import biblepaylib
        return biblepaylib.SHIM_serialise_for_biblepayd(self.serialise())
