...
def is_valid_biblepay_address(address, network='mainnet'):
    ...
    biblepay_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Biblepay address
        return False

    if (address_version != biblepay_version):
        return False
...
# shims 'til we can fix the biblepayd side
def SHIM_serialise_for_biblepayd(watchman_hex):
    from models import BIBLEPAYD_GOVOBJ_TYPES
    # unpack
    obj = deserialise(watchman_hex)

    # shim for biblepayd
    govtype = obj[0]

    # add 'type' attribute
    obj[1]['type'] = BIBLEPAYD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in biblepayd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # biblepayd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    biblepayd_hex = serialise(obj)
    return biblepayd_hex


# shims 'til we can fix the biblepayd side
def SHIM_deserialise_from_biblepayd(biblepayd_hex):
    from models import BIBLEPAYD_GOVOBJ_TYPES
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('biblepay.conf')
