...
class GovernanceObject(BaseModel):
    ...
    def import_gobject_from_biblepayd(self, biblepayd, rec):
        ...
        # shim/biblepayd conversion
        object_hex = biblepaylib.SHIM_deserialise_from_biblepayd(object_hex)
        objects = biblepaylib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from biblepayd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from biblepayd, with every run
	    ...
	    if subclass(**newdikt).is_valid(biblepayd) is False:
    ...
    def vote(self, biblepayd, signal, outcome):
        import biblepaylib
        ...
        output = biblepayd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = biblepaylib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(biblepayd, signal)

    def sync_network_vote(self, biblepayd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = biblepayd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = BIBLEPAYD_GOVOBJ_TYPES['proposal']
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/BiblepayDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import biblepaylib
        obj_data = biblepaylib.SHIM_serialise_for_biblepayd(self.serialise())
    ...
    def prepare(self, biblepayd):
        try:
            object_hash = biblepayd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = BIBLEPAYD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not biblepaylib.is_valid_biblepay_address(addr, config.network):
    ...
    def hash(self):
        import biblepaylib
        return biblepaylib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = BIBLEPAYD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, biblepayd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - biblepayd.WATCHMAN_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, biblepayd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - biblepayd.WATCHMAN_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, biblepayd):
        now = int(time.time())
        return (self.created_at < (now - biblepayd.WATCHMAN_WATCHDOG_MAX_SECONDS))

    def is_valid(self, biblepayd):
        if self.is_expired(biblepayd):
            return False

        return True

    def is_deletable(self, biblepayd):
        if self.is_expired(biblepayd):
            return True
...
def db_models():
    """ Return a list of Watchman DB models. """
    models = [
        GovernanceObject,
        Setting,
        Proposal,
