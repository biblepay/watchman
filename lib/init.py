...
def is_database_correctly_configured():
    ...
    cannot_connect_message = "Cannot connect to database. Please ensure database service is running and user access is properly configured in 'watchman.conf'."
...
def has_biblepay_conf():
    import config
    import io

    valid_biblepay_conf = False

    # ensure biblepay_conf exists & readable
    #
    # if not, print a message stating that Biblepay Core must be installed and
    # configured, including JSONRPC access in biblepay.conf
    try:
        f = io.open(config.biblepay_conf)
        valid_biblepay_conf = True
    except IOError as e:
        print(e)

    return valid_biblepay_conf
...
def main():
    ...
    if not has_biblepay_conf():
        print("BiblepayCore must be installed and configured, including JSONRPC access in biblepay.conf")
        sys.exit(1)
