...
class BiblepayDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some biblepayd methods
    ...
    @classmethod
    def from_biblepay_conf(self, biblepay_dot_conf):
        from biblepay_config import BiblepayConfig
        config_text = BiblepayConfig.slurp_config_file(biblepay_dot_conf)
        creds = BiblepayConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = biblepaylib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = biblepaylib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    @property
    def WATCHMAN_WATCHDOG_MAX_SECONDS(self):
